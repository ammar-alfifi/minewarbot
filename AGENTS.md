...
## العمل المشترك عبر OpenCode
كل أعضاء الفريق يستخدمون OpenCode، والملفات التالية مُودَعة في الريبو فيعمل المساعد بنفس القواعد عند الجميع:
- `AGENTS.md` — هذه التعليمات، تُحمَّل تلقائياً في كل جلسة (لذا لا تكرّر القواعد في المحادثة).
- `opencode.jsonc` — صلاحيات مشتركة تمنع قراءة/تعديل أي `*.env`، دون تعطيل النشر التلقائي.
- `.opencode/commands/*.md` — أوامر جاهزة للفريق: `/update-telegram` (البناء والنشر كاملاً) و`/review` و`/feature` و`/check`.
- لا تُودِع أي توكن أو سر؛ الأسرار على Cloudflare ومحلياً في `.env` المستثنى من Git.
- عند إضافة ميزة: فرع `feature/…` ثم Pull Request ليراجعه العضو الآخر قبل الدمج في `main`.
