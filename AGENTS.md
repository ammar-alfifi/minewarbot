...
### الوكيل يتولّى الدورة التقنية كاملة
الأعضاء مبتدئان؛ الوكيل مسؤول عن كل الجوانب التقنية بلا انتظار تعليمات:
- `/sync` في بداية كل مهمة لجلب آخر تغييرات `main`.
- تنفيذ المهمة على فرع من `main`، ثم الاختبار والبناء.
- فتح Pull Request ومراجعته ودمجه في `main` (`gh pr merge`) بعد نجاح الفحص، ثم حذف الفرع.
- النشر على Cloudflare وتحديث webhook حسب قسم النشر أعلاه، وإبقاء Git والإنتاج متطابقين.
- حل أي تعارض أو خطأ بنفسه، وعدم سؤال الأمثلة عن تفاصيل تقنية.

لا يسأل الوكيل عن: فروق تقنية، أوامر Git، تنظيم الملفات، أو أخطاء البناء.  
يسأل فقط عن: قرار يغيّر تصميم اللعبة، أو عندما يحتاج سرّاً/توكن لا يملكه.
